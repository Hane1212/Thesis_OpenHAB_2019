# 2019-04-24 Rule without trigger
            ...
            if '[INFO ]' in line[i]:
                while startTime < timeLog < stopTime:
                    ...
                    if 'START' in line[i]:
                        count = count+1
                        # print(count)
                        # rule_ = SubElement(rule, 'rule', OrderedDict([('state', 'START'), ('rule', tagRule)]))
                        rule_ = SubElement(rule, 'rule', OrderedDict([('Order', str(count)), ('state', 'START'), ('rule', tagRule)]))
                        ruleGroup[tagRule] = rule_
                    elif 'FINISH' in line[i]:
                        SubElement(rule, 'rule', OrderedDict([('state', 'FINISH'), ('rule', tagRule)]))
                        ...
                        SubElement(state, 'state', OrderedDict([('state', tagRule), ('item', value)]))
                    i=i+1
                    timeLog = getTime(line[i])
                if timeLog > stopTime:
                    i = len(line)
...
if __name__ == "__main__":
    ...
    # trace_test = getConfig('FILE_DIR', 'trace_test')
    # writeTrace(trace_test, openHABlog_test)
    openHABlog = getConfig('FILE_DIR', 'openHABlog')
    trace = getConfig('FILE_DIR', 'trace')
    writeTrace(trace, openHABlog)
